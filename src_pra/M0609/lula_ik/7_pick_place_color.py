...
USD_PATH         = str(M0609_DIR / "Collected_m0609_camera/m0609_camera_nocube.usd")
...
PLACE_XY_BY_COLOR = {
    "green": GREEN_PLACE_XY,
    "blue": BLUE_PLACE_XY,
}
PLACE_XY = np.full(2, np.nan)

COLOR_TOPIC = "/m0609/detected_color"
COLOR_NAME_BY_CODE = {1: "blue", 2: "green"}

FLOOR_Z = 0.0  # 실제 바닥 윗면 높이가 다르면 수정
# 높이
...
#  회전 유틸
# ══════════════════════════════════════════════════════════════
def create_place_markers(scene):
    marker_colors = {
        "green": np.array([0.0, 1.0, 0.0]),
        "blue":  np.array([0.0, 0.0, 1.0]),
    }

    for color_name, xy in PLACE_XY_BY_COLOR.items():
        marker_name = f"{color_name}_place_marker"
        if scene.object_exists(marker_name):
            continue
        ...
        scene.add(
            VisualCuboid(
                ...
                name=marker_name,
                position=np.array([xy[0], xy[1], FLOOR_Z + 0.001]),
                size=1.0,
                scale=np.array([0.10, 0.10, 0.002]),
                color=marker_colors[color_name],
            )
        )

def create_pick_cube(scene, color_name=None):
    colors = {
        "green": np.array([0.0, 1.0, 0.0]),
        "blue":  np.array([0.0, 0.0, 1.0]),
    }
    ...
    if color_name is None:
        color_name = np.random.choice(["green", "blue"])
    if color_name not in colors:
        raise ValueError("color_name은 'green' 또는 'blue'여야 합니다.")

    cube_size = PICK_Z  # 0.05 m: 큐브 윗면이 PICK_Z에 오도록 설정
    cube = scene.add(
        DynamicCuboid(
            prim_path="/World/pick_cube",
            name="pick_cube",
            position=np.array([PICK_XY[0], PICK_XY[1], cube_size / 2]),
            size=cube_size,
            color=colors[color_name],
            mass=0.05,
        )
    )
    print(f"   pick cube    {color_name} at {PICK_XY}")
    return cube, color_name

class ColorSubscriber(Node):
    def __init__(self):
        super().__init__("m0609_pick_place_color_sub")
        self.waiting = False
        self.color_code = None

        # BEST_EFFORT 구독은 BEST_EFFORT/RELIABLE 발행자와 모두 연결 가능
        qos = QoSProfile(depth=1, reliability=ReliabilityPolicy.BEST_EFFORT)
        self.create_subscription(Int32, COLOR_TOPIC, self._on_color, qos)

    def _on_color(self, msg):
        code = int(msg.data)
        if self.waiting and code in COLOR_NAME_BY_CODE:
            self.color_code = code

    def start_detection(self):
        self.color_code = None
        self.waiting = True
        print(f"   waiting for {COLOR_TOPIC} (1=blue, 2=green)")
...
class PickPlaceFSM:
    ...
    NAMES = ["APPROACH", "DETECT", "DESCEND", 
             "GRASP", "LIFT", "MOVE", "LOWER", "RELEASE", "DONE"]
    DETECT_STATE = 1
    ...
    def _build_waypoints(self):
        ...
        self.waypoints = [
            ...
            np.array([px, py, APPROACH_HEIGHT]),  # 1 DETECT
    ...
    def advance(self):
        """한 스텝 진행한다"""
        if self.state == self.DETECT_STATE:
            if self.start is None:
                self.start = get_tcp_pose(self._robot)
                self.goal = self.start.copy()
                self.n_steps = 1
                self.gripper = "open"
                print("   [1] DETECT    waiting for color result")
            return
    ...
    def choose_color(self, code):
        if self.state != self.DETECT_STATE:
            return False

        color = COLOR_NAME_BY_CODE.get(code)
        if color is None:
            return False

        PLACE_XY[:] = PLACE_XY_BY_COLOR[color]
        self._build_waypoints()
        print(f"   detected     {color} -> place {vec(PLACE_XY)}")
        self._next()  # DETECT -> DESCEND
        return True
...
def main():
    ...
    while simulation_app.is_running():
        ...
        if is_playing and not was_playing:
            # 이전 Play의 큐브를 씬과 USD에서 제거
            if world.scene.object_exists("pick_cube"):
                world.scene.remove_object("pick_cube")

            # 이번 Play의 집기 위치와 큐브 색을 새로 선택
            PICK_XY[:] = [
                np.random.uniform(0.20, 0.45),
                np.random.uniform(-0.40, 0.40),
            ]
            color_sub.stop_detection()
            task._cube, _ = create_pick_cube(world.scene)
            PLACE_XY[:] = np.nan
            # 새 큐브의 물리 객체 초기화
            world.reset()
            robot.initialize()
            ...
            # 상태 기계가 새 PICK_XY를 목표로 쓰도록 갱신
            fsm._build_waypoints()
            fsm.reset()
            step = 0
            print()

        if is_playing and fsm.state == fsm.DETECT_STATE:
            if not color_sub.waiting:
                color_sub.start_detection()
            elif color_sub.color_code is not None:
                if fsm.choose_color(color_sub.color_code):
                    color_sub.stop_detection()
                    print_target_info(target_quat)
        elif not is_playing:
            color_sub.stop_detection()
